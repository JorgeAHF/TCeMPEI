...
class StrandType(Base):
    ...
    E_MPa = Column("e_mpa", Float, nullable=False)
    Fu_default = Column("fu_default", Float, nullable=False)
...
class CableStateVersion(Base):
    ...
    Fu_override = Column("fu_override", Float)
...
class Acquisition(Base):
    ...
    Fs_Hz = Column("fs_hz", Float, nullable=False)
    notes = Column(Text)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    created_by_user_id = Column(Integer, ForeignKey("users.id"))
...
class WeighingMeasurement(Base):
    ...
    measured_temperature_C = Column("measured_temperature_c", Float)
    notes = Column(Text)
